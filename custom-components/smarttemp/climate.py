            ...
            async def _async_task():
                ...
                for device_mac in target_macs:
                    ...
                    if f"{device_mac}_registered" not in known_devices:
                        _LOGGER.info(f"Triggering Discovery via pair_key for {device_mac}")
                        ...
                        for i in range(zone_count):
                            _LOGGER.info(f"Creating Zone {i+1} for {device_mac}")
...
class SmartTempAC(SmartTempBase):
    ...
    def hvac_mode(self):
        ...
        if isinstance(mode, list): mode = mode[0] if mode else 0
    ...
    def fan_mode(self):
        ...
        if isinstance(speed, list): speed = speed[0] if speed else 0
    ...
    async def async_set_fan_mode(self, fan_mode):
        ...
        await self.hub.send_smarttemp_command(self._mac, {"fan_speed": mapping.get(fan_mode, 0)})

    @property
    def preset_mode(self):
        """Report Fan Policy (Auto vs Continuous)."""
        policy = self.coordinator.get_field(self._mac, "fan_mode", 0)
        if isinstance(policy, list): policy = policy[0] if policy else 0
    ...
    """Main AC Unit with Speed (fan_mode) and Policy (preset_mode)."""
    ...
    def __init__(self, coordinator, hub, mac):
        ...
        # Mapping to 'fan_speed' in protocol (0-3)
        self._attr_fan_modes = [FAN_AUTO, FAN_LOW, FAN_MEDIUM, FAN_HIGH]
        
        # Mapping to 'fan_mode' in protocol (0=Auto, 1=Continuous)
        self._attr_preset_modes = ["Auto Fan", "Continuous Fan"]
    ...
    def fan_mode(self):
        """Speed mapping."""
        speed = self.coordinator.get_field(self._mac, "fan_speed", 0)
        # Handle list if speed is wrapped
        if isinstance(speed, list): speed = speed[0] if speed else 0
...
class SmartTempZone(SmartTempBase):
    ...
    def __init__(self, coordinator, hub, mac, zone_idx):
        super().__init__(coordinator, hub, mac)
        self._zone_idx = zone_idx  # 0-indexed for data, 1-indexed for commands
        self._zone_num = zone_idx + 1
        self._attr_unique_id = f"{mac}_zone_{self._zone_num}"
    ...
    async def async_set_hvac_mode(self, hvac_mode):
        ...
        if new_status == 1:
            ...
            if main_mode == 0: # If system is currently OFF
                _LOGGER.info(f"Opening {self._attr_name}: Powering up main unit to Auto.")
    ...
    async def async_set_temperature(self, **kwargs):
        ...
        temp = int(kwargs.get(ATTR_TEMPERATURE) * 10)
